...
$app->get('/process/:keyword/:count', function ($keyword, $count) use ($app) {
    ...
    $tweets = $twitter->requestTweet('search/tweets', array('q' => "$keyword  -http -filter:retweets -filter:links -filter:replies -filter:images lang:en", 'result_type' => 'recent', 'count' => $count, 'exclude_replies' => true));
